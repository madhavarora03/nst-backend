...
from app.models.user import UserModel, user_collection, UserResponseModel, UserBody, UserPublicModel
...
async def login(response: Response, user: UserBody = Body(...)):
    try:
        ...
        db_user = user_collection.find_one({"$or": [{"email": identifier}, {"username": identifier}]})
        if not db_user or not verify_password(user.password, db_user["password"]):
            print(db_user["password"], user.password)
            raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Incorrect email or password")

        access_token = create_access_token({"sub": db_user["email"]}, timedelta(minutes=60 * 24))
